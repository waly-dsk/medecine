...
class CursusController extends Controller
{
    ...
    {
        return view('menu.vie_estudiantine.cursus_init');
    }
    ...
    public function get(Request $request)
    {
        ...
        // La requête permet d'obtenir le cursus d'un étudiant (avec son matricule) 
        /**
         * Il faut remarquer la DOUBLE JOINTURE
         
         * La première jointure est entre les tables 'inscriptions' et 'decision_annee_acas' 
         * en utilisant les colonnes 'matricule' et 'anneeAca'. Cela permet de récupérer les 
         * décisions prises pour chaque année académique pour chaque étudiant.
         * 
         * La deuxième jointure est entre les tables 'inscriptions' et 'classes' en utilisant 
         * la colonne 'codeClasse'. Cela permet de récupérer les libellés des classes auxquelles 
         * l'étudiant est inscrit pour chaque année académique.
         */
        $cursus = DB::table('inscriptions')
            ->join('decision_annee_acas', function ($join) {
                $join->on('inscriptions.matricule', '=', 'decision_annee_acas.matricule')
                    ->on('inscriptions.anneeAca', '=', 'decision_annee_acas.anneeAca');
            })
            ->join('classes', 'inscriptions.codeClasse', '=', 'classes.codeClasse')
            ->where('decision_annee_acas.matricule', '=', $matricule)
            ->select(
                'inscriptions.anneeAca',
                'classes.libClasse',
                'decision_annee_acas.decision'
            )
            ->orderBy('inscriptions.anneeAca', 'desc')
            ->get();

        return view('menu.vie_estudiantine.cursus_get', compact(['etudiant', 'cursus']));
    }
